...
import { fetchCollectionUnity, sendEmail } from '../../../../fetch'
...
const NotesComponent = ({
  ...
  const extractChartData = (serverData) => {
    return serverData.map((item, i) => {
      if (item.chart_image)
        return (
          <MenuItem onClick={(e) => e.preventDefault()} key={i}>
  ...
  React.useEffect(() => {
    console.log(showSaveChange, 'value updated', value)
    // if (value === null) {
    //   setShowSaveChange(false);
    // } else {
    //   setShowSaveChange(true);
    // }
    if (showSaveChange === null && value === null) {
      // setShowSaveChange(false);
    } else if (showSaveChange === null && value) {
      setShowSaveChange(false)
    } else {
      setShowSaveChange(true)
    }
  ...
  const [showSaveChange, setShowSaveChange] = useState(null)
